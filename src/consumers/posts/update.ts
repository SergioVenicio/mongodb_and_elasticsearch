...
const queue = process.env.POST_UPDATE_QUEUE as string
...
class CreatePostConsumer {
  ...
  async consume(msg: amqplib.Message|null) {
    ...
    await this.repository.update(post as Post)
  }
}
...
async function main () {
  ...
  await chan.bindQueue(queue, exchange, 'UPDATE')
